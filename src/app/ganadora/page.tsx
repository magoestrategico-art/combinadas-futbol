...
export default function GanadoraPage() {
  ...
  const generarCombinadaAutomatica = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPartidos(data.data.partidos); // Actualizar el estado 'partidos' con los datos generados
      } else {
  ...
  const guardarAlHistorial = async () => {
    ...
    console.log("Estado de partidos antes de guardar:", partidos);

    if (!partidos || partidos.length === 0) {
      console.error("Error: El estado 'partidos' está vacío o no se ha actualizado correctamente.");
      setSaveHistorialStatus("❌ Error: No hay partidos para guardar.");
      return;
    }
    ...
    try {
      ...
      console.log("Datos enviados a Firebase:", datosAGuardar);

      await addDoc(collection(db, "historialCombinadas"), datosAGuardar);
