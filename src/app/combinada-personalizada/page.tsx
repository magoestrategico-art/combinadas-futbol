...
// Reutilización del componente de combinada personalizada
...
const ligas = [
  ...
  { nombre: "Segunda División", id: 2016 },
];
const criterios = [
...
// Definir el tipo Resultado si no está definido
interface Resultado {
  liga: string;
  criterio: string;
  nombre: string;
...
export default function CombinadaPersonalizadaPage() {
  ...
  const handleChange = (i: number, campo: "liga" | "criterio", valor: string) => {
    const nueva = [...picks];
    nueva[i][campo] = valor;
    setPicks(nueva);
  };
  ...
  const generar = async () => {
    ...
    try {
      ...
      setError("Error de conexión" as unknown as string | null);
    } finally {
      setLoading(false);
    }
  };
  ...
  const guardarCombinada = async () => {
    ...
    try {
      if (resultado) {
        const equipos = resultado.map((r: Resultado) => r.nombre);
        ...
        {resultado && (
          <div className="mt-6">
            ...
            <ul className="space-y-2">
              {resultado && resultado.map((pick: Resultado, i: number) => (
