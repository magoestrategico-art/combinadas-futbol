...
interface CombinadaGuardada {
  ...
  partidos?: any[]; // Agregado para resolver el error
}
  ...
  const cargarCombinadas = async () => {
    try {
      ...
      const data = snapshot.docs.map(doc => {
        ...
        // Validar si el campo 'partidos' existe, si no, inicializarlo como un arreglo vacío
        return {
          id: doc.id,
          ...docData,
          partidos: docData.partidos || [] // Asegurar que 'partidos' siempre esté definido
        };
      }) as CombinadaGuardada[];

      console.log("Datos cargados desde Firebase:", data); // Log para depuración
      setCombinadas(data);
    } catch (error) {
